...
- Coverage floor raised from 50% to 75% (actual: 100%).
- `Makefile` removed in favor of `Taskfile.yml`.
- README updated for the `task`-based workflow and release process.
- CLAUDE.md pruned and restructured per Anthropic's CLAUDE.md best-practice
  guidance.
...
- `pytest` + `pytest-cov` with a 50% coverage floor.
- Type stubs (`cmod/_cmod.pyi`) and PEP 561 `py.typed` marker.
